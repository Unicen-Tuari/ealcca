<?php
class RouterConfig{
  public static $ACTION = 'action';
  public static $ACTION_INICIO = 'inicio';
  public static $ACTION_SOMOS = 'somos';
  public static $ACTION_NOTICIAS = 'noticias';
  public static $ACTION_CONTACTO = 'contacto';
  public static $ACTION_NACIONAL = 'nacional';
  public static $ACTION_INTER = 'inter';
  public static $ACTION_REGGAE = 'reggae';
  public static $ACTION_COMICS = 'comics';
  public static $ACTION_CALAVERAS = 'calaveras';
  public static $ACTION_CAMISAS = 'camisas';
  public static $ACTION_ACCESORIOS = 'accesorios';
  public static $ACTION_BUSOS = 'busos';
  public static $ACTION_MMA = 'mma';
  public static $ACTION_INSTRUMENTOS = 'instrumentos';
}

 ?>
